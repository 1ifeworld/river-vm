import {
  ...
  const submissionId = "bafyreiavsqke6rzj63xdm3uxdsbphwyval67ifr77rrqke7if5lcxksjki";
  const accept = true
  const reject = false
  
  async function createResponse(messageId: string, response: boolean): Promise<void> {
    ...
    const serializedChannelMessage =
      serializeMessageForHttp(genericResponseMessage);
